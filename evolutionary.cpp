...
unsigned maxNotImproving;
double mainFitnessNotEqual;
double greedyFitnessNotEqual;
        ...
        while(gen <= numGen && notImproving < maxNotImproving && duration < 600)
        {
            ...
            for(int i = 0; i < popSize; ++i)
            {
                if(abs(minObj - maxObj) < EPS)
                    ...
                    fitness[i] = mainFitnessNotEqual - (solutions[i].objective - minObj)/(maxObj - minObj);
                fitSum += fitness[i];
                assert(leq(fitness[i], mainFitnessNotEqual));
            }
            std::uniform_real_distribution<double> distrib(0.0, fitSum);
        ...
        for(int i = 0; i < popSize; ++i)
        {
            ...
            for(int j = 0; j < m; ++j)
            {
                if(eq(minVal, maxVal))
                    ...
                    fitness[j] = greedyFitnessNotEqual - ((edges[j].len - minVal)/(maxVal - minVal));
                fitSum += fitness[j];
                assert(leq(fitness[j], greedyFitnessNotEqual));
            }
            int chosen;
            Edge e;
            Solution sol;
